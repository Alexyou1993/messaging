...
class Messaging {
  ...
  Future<String>? send(Message message, bool? dryRun) {
    ...
    getUrlPatch!.then((String urlPatch) {
      bool? validateOnly;
      final Map<String, dynamic> request = <String, dynamic>{'message': message, 'validateOnly': validateOnly};
      request['message'] = copy;
      if(dryRun == true){
        request['validateOnly'] = true;
      }
      return messagingRequestHandler.invokeRequestHandler(FCM_SEND_HOST, urlPath, request);
  ...
  Future<BatchResponse>? sendAll(List<Message>? messages, bool? dryRun) {
    ...
    // Future<String>.value().then((String urlPath) {
    ...
    //       request['validateOnly'] = true;
    //     }
    //     final SubRequest? result;
    //     result.url = 'https://$FCM_SEND_HOST$urlPath';
    //     result.body = request;
    //     return result;
    //   });
    ...
    //       validateOnly = true;
    //     }
    //     Map<String, dynamic> request = <String, dynamic>{ 'message': message; 'validateOnly': validateOnly};
    //
    //     return {
...
MessagingOptions validateMessagingOptions(MessagingOptions options,) {
  ...
  if (optionsCopy.collapseKey!.isEmpty) {
    ...
    final String keyName = (options.toString().contains('contentAvailable')) ? 'contentAvailable' : 'content_available';
    throw FirebaseError.messaging(MessagingClientErrorCode.INVALID_OPTIONS,
        'Messaging options contains an invalid value for the "$keyName" property. Value must '
            'be a boolean.');
